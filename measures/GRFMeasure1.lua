...
function update( model, time )
    ...
    if calcn_l:com_pos().x < calcn_r:com_pos().y and normalized_GRF_l_y > prev_grf_l then
        if normalized_GRF_r_y < 1.05 or normalized_GRF_r_y > grf_threshold then
            cost2 = cost2 + 1.0
        end
    elseif calcn_r:com_pos().x < calcn_l:com_pos().y and normalized_GRF_r_y > prev_grf_r then
