function init( model, par, side )
    ...
    muscle_configs = {
        {name = "soleus", L = 0.6, alpha = 2.5, std = 0.01, min = 0.5, max = 2},  -- 206 gen 1.248, state controller working
        {name = "gastroc", L = 0.6, alpha = 2.5, std = 0.01, min = 0.5, max = 2}, 
    }
...
function update( model, time, controller )
    ...
    for muscle_name, muscle_pair in pairs(muscles) do
        ...
        if normalized_GRF_l_x > 0 then
            FMC_l = muscle_params.c * normalized_GRF_l_x * ( L_l - muscle_params.L )
        end
        ...
        if normalized_GRF_r_x > 0 then
            FMC_r = muscle_params.c * normalized_GRF_r_x * ( L_r - muscle_params.L )
        end
