function init( model, par, side )
    ...
    muscle_configs = {
        {name = "iliopsoas", L = 0.72, alpha = 1.0, std = 0.01, min = 0.0, max = 0.8},  -- 206 gen 1.248, state controller working
    }
    ...
    for _, config in ipairs(muscle_configs) do
        ...
        params[muscle_name] = {
            ...
            c = par:create_from_mean_std(muscle_name .. ".c", 1.0, 0.01, 0.0, 5.0),   -- 206 gen 1.248, state controller working
...
function update( model, time, controller )
    ...
    for muscle_name, muscle_pair in pairs(muscles) do
        ...
        if (L_l - muscle_params.L) < 0 then
            ...
            FMC_l = muscle_params.c * normalized_GRF_r_y * ( L_l - muscle_params.L )
        end
        ...
        if (L_r - muscle_params.L) < 0 then
            ...
            FMC_r = muscle_params.c * normalized_GRF_l_y * ( L_r - muscle_params.L )
        end
